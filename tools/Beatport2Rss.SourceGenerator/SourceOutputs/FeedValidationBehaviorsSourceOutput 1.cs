...
internal static class SubscriptionValidationBehaviorsSourceOutput
{
        ...
        IReadOnlyList<MediatorMessageInfo> mediatorMessageInfos)
    {
        ...
        foreach (var info in mediatorMessageInfos.OrderBy(i => i.Name))
        {
            ...
            builder.AppendLine($"internal sealed class {info.Name}SubscriptionValidationBehavior(ISubscriptionQueryRepository subscriptionQueryRepository) :");
            ...
            if (resultSymbol.IsGenericType)
            {
                ...
                builder.AppendLine($"    SubscriptionValidationBehavior<{info.Name}, {resultSymbol.Name}<{valueSymbol.Name}>, {valueSymbol.Name}>(subscriptionQueryRepository),");
                builder.AppendLine($"    IPipelineBehavior<{info.Name}, {resultSymbol.Name}<{valueSymbol.Name}>>");
            }
            else
            {
                builder.AppendLine($"    SubscriptionValidationBehavior<{info.Name}, {resultSymbol.Name}>(subscriptionQueryRepository),");
        ...
        context.AddSource("SubscriptionValidationBehaviors.g.cs", builder.ToString());
    }
}