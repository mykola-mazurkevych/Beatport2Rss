...
internal static class UserValidationBehaviorsSourceOutput
{
        ...
        IReadOnlyList<MediatorMessageInfo> mediatorMessageInfos)
    {
        ...
        foreach (var info in mediatorMessageInfos.OrderBy(i => i.Name))
        {
            ...
            builder.AppendLine($"internal sealed class {info.Name}UserValidationBehavior(IUserQueryRepository userRepository) :");
            ...
            if (resultSymbol.IsGenericType)
            {
                ...
                builder.AppendLine($"    UserValidationBehavior<{info.Name}, {resultSymbol.Name}<{valueSymbol.Name}>, {valueSymbol.Name}>(userRepository),");
                builder.AppendLine($"    IPipelineBehavior<{info.Name}, {resultSymbol.Name}<{valueSymbol.Name}>>");
            }
            else
            {
                builder.AppendLine($"    UserValidationBehavior<{info.Name}, {resultSymbol.Name}>(userRepository),");
