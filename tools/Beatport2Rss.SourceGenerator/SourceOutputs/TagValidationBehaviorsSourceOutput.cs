...
internal static class TagValidationBehaviorsSourceOutput
{
        ...
        IReadOnlyList<MediatorMessageInfo> mediatorMessageInfos)
    {
        ...
        foreach (var info in mediatorMessageInfos.OrderBy(i => i.Name))
        {
            ...
            builder.AppendLine($"internal sealed class {info.Name}TagValidationBehavior(ITagQueryRepository tagQueryRepository) :");
            ...
            if (resultSymbol.IsGenericType)
            {
                ...
                builder.AppendLine($"    TagValidationBehavior<{info.Name}, {resultSymbol.Name}<{valueSymbol.Name}>, {valueSymbol.Name}>(tagQueryRepository),");
                builder.AppendLine($"    IPipelineBehavior<{info.Name}, {resultSymbol.Name}<{valueSymbol.Name}>>");
            }
            else
            {
                builder.AppendLine($"    TagValidationBehavior<{info.Name}, {resultSymbol.Name}>(tagQueryRepository),");
        ...
        context.AddSource("TagValidationBehaviors.g.cs", builder.ToString());
    }
}