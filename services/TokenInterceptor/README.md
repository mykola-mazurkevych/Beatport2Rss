...
The TokenInterceptor service is a specialized token provider that intercepts Beatport access tokens from HTTP responses during authentication. It exposes a gRPC endpoint (`GetToken`) that returns cached tokens with automatic refresh capability.
...
```bash
cd services/TokenInterceptor/src/Beatport2Rss.TokenInterceptor
dotnet run
```
...
```bash
grpcurl -plaintext -d '{}' localhost:5000 Beatport2Rss.TokenInterceptor.TokenService/GetToken
```
...
```bash
docker build -f services/TokenInterceptor/src/Beatport2Rss.TokenInterceptor/Dockerfile -t beatport-token-interceptor .
```
